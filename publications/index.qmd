...
icon.btn.html<-data.frame(
  ...
  <span style=\"font-size: 1em;\"> Other URI </span> ](",
  "){.btn .btn-secondary role=\"button\"}  "))
...
badges.html<-data.frame(
  citations=c(
    ...
    "-brightgreen?logo=googlescholar\">"
  )
)
...
for (doi.ind in 1:length(doi.list)) {
  ...
  if (doi.list[doi.ind]=="10.1016/j.buildenv.2021.107928"){
    cat(paste0("::: grid \n", #grid-2
               "::: {.g-col-10 .vh-left .just-text} \n", #g-col-10-2
               "> Elsevier Building and Environment journal, [invitation-based special initiative paper](https://doi.org/10.1016/j.buildenv.2015.08.012) \n",
               "::: \n", #close g-col-10-2 
               "::: \n")) #close grid-2 
  }
